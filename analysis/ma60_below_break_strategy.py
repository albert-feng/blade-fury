...
def quant_stock(stock_number, stock_name, **kwargs):
    ...
    strategy_name = f"ma{ma_window}_below60_break"

    below_days = 60
    quant_count = ma_window + below_days + 50

    sdt = SDT.objects(Q(stock_number=stock_number) & Q(date__lte=qr_date)).order_by(
        "-date"
    )[:quant_count]
    if not sdt or len(sdt) < (ma_window + below_days + 1):
        return

    trading_data = format_trading_data(sdt)
    df = DataFrame(trading_data)
    if len(df) < (ma_window + below_days + 1):
        return
    ...
    prev_days = df.iloc[-(below_days + 1) : -1]
    if prev_days["ma"].isna().any():
        return

    if not (prev_days["close_price"] < prev_days["ma"]).all():
        return

    if today["close_price"] <= today["ma"]:
        return

    increase_rate = (
        round(today["pct_change"] * 100, 2) if not pd.isna(today["pct_change"]) else 0
    )
...
def setup_argparse():
    parser = argparse.ArgumentParser(
        description="连续60日收盘低于N日均线后当日收盘上穿策略"
    )
    parser.add_argument(
        "-n", action="store", dest="ma_window", required=True, help="均线周期"
    )
    parser.add_argument(
        "-t", action="store", dest="qr_date", required=False, help="计算日期"
    )
